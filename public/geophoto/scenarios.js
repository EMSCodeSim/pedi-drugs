// scenarios.js
// Scenario loader, canvas, overlays, saving. No AI fetch/upload writes.

import { getFirebase, ensureAuthed, getStorageInfo, toStorageRefString, getBlobFromRefString, candidateOriginals } from "./firebase-core.js";
import { ref as dbRef, get, set, remove, onValue } from "https://www.gstatic.com/firebasejs/10.12.4/firebase-database.js";
import { ref as stRef, getDownloadURL, uploadBytesResumable, uploadBytes, deleteObject } from "https://www.gstatic.com/firebasejs/10.12.4/firebase-storage.js";
...
export function setRootPill(text) {
  $("rootPill").textContent = text;
}
...
export async function getCompositeDataURL(maxEdge = 1600, quality = 0.95) {
  // Re-rasterize the canvas to a scaled JPEG (small, upload-friendly)
...
export async function saveResultBlobToStorage(blob) {
  // Save under /ai/results without touching stops. The AI module asks user before adding.
  if (!current || stopIndex < 0) throw new Error("No stop selected");
  const { storage } = getFirebase();
  const ts = Date.now();
  ...
  // resumable upload with a small timeout
  await new Promise((resolve, reject) => {
    ...
    task.on("state_changed",
      () => {},
      (err) => { clearTimeout(killer); reject(err); },
...
export function wireScenarioUI() {
  ...
  // Export/save image (small JPEG)
  const EDIT_MAX_EDGE = 1280, EDIT_THUMB_EDGE = 480, EDIT_JPEG_QUALITY = 0.84;

  async function dataURLFromCanvasScaled(canvas, maxEdge, quality) {
    const raw = canvas.toDataURL({ format: "jpeg", quality: 1 });
    const img = new Image(); img.decoding = "async"; img.src = raw; await img.decode();
    const w = img.naturalWidth, h = img.naturalHeight;
    const s = Math.min(1, maxEdge / Math.max(w, h));
    const outW = Math.round(w * s), outH = Math.round(h * s);
  ...
  $("saveImage").onclick = async () => {
    try {
      ...
      const fullRef = stRef(getFirebase().storage, `${base}.jpg`);
      const thumbRef = stRef(getFirebase().storage, `scenarios/${current.id}/thumbs/${ts}_edited_${stopIndex}.jpg`);
      ...
      await new Promise((resolve, reject) => {
        ...
        const killer = setTimeout(() => { try { task.cancel(); } catch {} reject(Object.assign(new Error("save/timeout-upload"), { code: "save/timeout-upload" })); }, 45_000);
        task.on("state_changed", () => {}, (e) => { clearTimeout(killer); reject(e); }, () => { clearTimeout(killer); resolve(); });
      });
      try { await uploadBytes(thumbRef, thumbBlob, metaThmb); } catch {}
      ...
      setStatus("Saved edited image ✓");
      renderThumbs();
    } catch (e) {
      showError(e.message || String(e));
      $("metaMsg").textContent = "Save failed: " + (e?.message || e);
    } finally { showLoad(false); }
  };

  // Overlay shelf / brushes / selection tools (unchanged UI behavior)
