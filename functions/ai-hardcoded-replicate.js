...
function initFirebaseAdmin() {
  ...
  const saJson = process.env.FIREBASE_SERVICE_ACCOUNT_JSON; // full JSON string (one line)
  const bucketName = process.env.FIREBASE_STORAGE_BUCKET;   // e.g. "your-bucket.appspot.com"
...
async function downloadFromFirebase(storagePath) {
  ...
  if (!admin) {
    throw new Error(
      "Firebase Admin not configured (set FIREBASE_SERVICE_ACCOUNT_JSON and FIREBASE_STORAGE_BUCKET)."
    );
  }
...
exports.handler = async (event) => {
  try {
    ...
    const { storagePath, imageUrl, dataUrl, prompt, negativePrompt, imageStrength } = body;
    ...
    if (storagePath) {
      ...
      const badHost =
        /googleusercontent\.com$/.test(urlObj.hostname) ||
        /appspot\.com$/.test(urlObj.hostname);
      if (badHost && !/firebasestorage\.googleapis\.com\/v0\//.test(imageUrl)) {
        throw new Error(
          "imageUrl looks like a 'download/storage/v1' link that needs Google auth. " +
          "Use getDownloadURL(...) (v0 with ?alt=media&token=...) or pass storagePath instead."
        );
      }
    ...
    // 3) Prediction (SDXL img2img)
    const strength = typeof imageStrength === "number"
    ...
    const output = Array.isArray(prediction.output) && prediction.output.length
      ? prediction.output[0]
      : prediction.output;
